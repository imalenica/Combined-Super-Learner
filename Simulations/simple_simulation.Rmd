...
information loss. The best known IC is the Akaike IC
$$AIC=−2ln(l)+2k$$
...
run_auto_arima <- function(df, outcome) {
  ...
  fit_list <- lapply(samples, function(x){
    ...
    # 6. make relevant results pretty and return them
...
```
```{r, mimic-autoarima-open, echo = FALSE}
library(here)
library(forecast)
...
Let's view the first 6 rows of the outcome specific tables that we mentioned
above.

```{r, mimic-autoarima-view, echo = FALSE}
kable(head(arima_abpmean$coefficients, 10), format = "latex",
      caption = "ARIMA Coefficients with Mean BP Outcome")
acc_mean <- t(head(arima_abpmean$accuracy, 10))  
colnames(acc_mean) <- acc_mean[1,]
acc_mean <- acc_mean[-1,]      
kable(acc_mean, format = "latex",
      caption = "ARIMA Accuracy with Mean BP Outcome")

kable(head(arima_abpsys$coefficients, 10), format = "latex",
      caption = "ARIMA Coefficients with Systolic BP Outcome")
acc_sys <- t(head(arima_abpsys$accuracy, 10))  
colnames(acc_sys) <- acc_sys[1,]
acc_sys <- acc_sys[-1,]
kable(acc_sys, format = "latex",
      caption = "ARIMA Accuracy with Systolic BP Outcome")

kable(head(arima_abpdias$coefficients, 10), format = "latex",
      caption = "ARIMA Coefficients with Diastolic BP Outcome")
acc_dias <- t(head(arima_abpdias$accuracy, 10))  
colnames(acc_dias) <- acc_dias[1,]
acc_dias <- acc_dias[-1,]      
kable(acc_dias, format = "latex",
...
```{r, mimic-autoarima-plots, echo = FALSE}
...
plot_multi(multi_forecast = arima_abpsys$plot_data[[1]]$multi_forecast,
           test_outcome = arima_abpsys$plot_data[[1]]$test_outcome)
plot_onestep(onestep_forecast = arima_abpsys$plot_data[[1]]$onestep_forecast,
             test_outcome = arima_abpsys$plot_data[[1]]$test_outcome)
plot_all(onestep_forecast = arima_abpsys$plot_data[[1]]$onestep_forecast,
         multi_forecast = arima_abpsys$plot_data[[1]]$multi_forecast,
         test_outcome = arima_abpsys$plot_data[[1]]$test_outcome)

plot_multi(multi_forecast = arima_abpdias$plot_data[[1]]$multi_forecast,
           test_outcome = arima_abpdias$plot_data[[1]]$test_outcome)
plot_onestep(onestep_forecast = arima_abpdias$plot_data[[1]]$onestep_forecast,
             test_outcome = arima_abpdias$plot_data[[1]]$test_outcome)
plot_all(onestep_forecast = arima_abpdias$plot_data[[1]]$onestep_forecast,
         multi_forecast = arima_abpdias$plot_data[[1]]$multi_forecast,
         test_outcome = arima_abpdias$plot_data[[1]]$test_outcome)         
```

```{r, mimic-autoarima-plots-residuals, echo = FALSE}
plot(arima_abpmean$plot_data[[1]]$multi_forecast,
     main = "Multi-step ARIMA Forecasts with Mean BP Outcome")
plot(arima_abpmean$plot_data[[1]]$onestep_forecast,
     main = "One-step ARIMA Forecasts with Mean BP Outcome")

plot(arima_abpsys$plot_data[[1]]$multi_forecast,
     main = "Multi-step ARIMA Forecasts with Systolic BP Outcome")
plot(arima_abpsys$plot_data[[1]]$onestep_forecast,
     main = "One-step ARIMA Forecasts with Systolic BP Outcome")

plot(arima_abpdias$plot_data[[1]]$multi_forecast,
     main = "Multi-step ARIMA Forecasts with Diastolic BP Outcome")
plot(arima_abpdias$plot_data[[1]]$onestep_forecast,
     main = "One-step ARIMA Forecasts with Diastolic BP Outcome")
```
...
- What's up with the repeated subject ID's with different ages? They are
clearly different subjects...
- Maybe the computational folks should share GitHub repos?
