...
slice_rows <- function(dt){
  df <- data.frame(dt)
  data <- df %>% dplyr::group_by(id) %>% slice(11:(n()-46))
...
save(individual, file=here("Data", "individual_mean_full.Rdata"), compress=T)

individual <- slice_rows(individual)
individual <- process_cols(individual)
save(individual, file=here("Data", "individual_mean.Rdata"), compress=T)
end_ind_mean <- proc.time()-start
...
save(historical, file=here("Data", "historical_mean_full.Rdata"), compress=T)
...
save(historical, file=here("Data", "historical_mean.Rdata"), compress=T)
end_hist_mean <- proc.time()-start
...
save(individual, file=here("Data", "individual_median_full.Rdata"), compress=T)

individual <- slice_rows(individual)
individual <- process_cols(individual)
save(individual, file=here("Data", "individual_median.Rdata"), compress=T)
end_ind_median <- proc.time()-start
...
save(historical, file=here("Data", "historical_median_full.Rdata"), compress=T)

historical <- slice_rows(historical)
historical <- process_cols(historical)
historical <- process_data(historical, strata=c("admission_type_descr","sex"))
save(historical, file=here("Data", "historical_median.Rdata"), compress=T)
end_hist_median <- proc.time()-start