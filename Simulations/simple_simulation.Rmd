...
run_auto_arima <- function(df, outcome) {
  ...
  coefficients <- do.call("rbind.fill",
  lapply(fit_list, function(x) data.frame(as.list(x$coefficients))))
  coefficients <- data.frame(subject_id = samples, coefficients)

  accuracy <- do.call("rbind.fill",
  lapply(fit_list, function(x) data.frame(as.list(x$accuracy))))
...
- MIMIC data without the time gap!! :O
