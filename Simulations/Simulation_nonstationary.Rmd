...
\vspace{.2in}
```{r editdata, echo = FALSE, eval=FALSE, message = FALSE, warning = FALSE}
...
* Individual models for each inidvidual with at least one hypotensive episode. 
...
save(sub_samples, file = here::here("Results/Nonstationary_ts", "sub_samples.Rdata"),
     compress = TRUE)
save(df, file = here::here("Results/Nonstationary_ts", "data.Rdata"),
     compress = TRUE)
```
...
load(here::here("Results/Nonstationary_ts","data.Rdata"))
...
plot_forecast(type = "individual", id = 1012)
plot_forecast(type = "pooled", id = 1012)
...
`t` times. As such, the resulting time-series is a mixture of 
the individual and pooled model, mixed at pre-specified time-point. 
...
In addition to the combined (non-stationary) time-series, we also generate
simulated data sets for the individual and pooled time-series models. 
...
id<-20403
test_sample <- run_simulation(id=id, noise = TRUE, t = 1)
...
###############################
nonstat_sub_df<-rbind.data.frame(new_sub_df, test_sample$dat)
save(nonstat_sub_df, file = here::here("Results/Nonstationary_ts/t1", paste0("full_t1_test_sample", id, ".Rdata")), compress = TRUE)
save(test_sample, file = here::here("Results/Nonstationary_ts/t1", paste0("test_t1_sample", id, ".Rdata")), compress = TRUE)

###############################
# Individual time-series
###############################
dat <- test_sample$dat
dat$abpmean <- test_sample$abpmean_individual
ind_sub_df<-rbind.data.frame(new_sub_df, dat)
save(ind_sub_df, file = here::here("Results/Nonstationary_ts/t1", paste0("ind_t1_test_sample", id, ".Rdata")), compress = TRUE)

###############################
# Pooled time-series
###############################
dat <- test_sample$dat
dat$abpmean <- test_sample$abpmean_pooled
pool_sub_df<-rbind.data.frame(new_sub_df, dat)
save(pool_sub_df, file = here::here("Results/Nonstationary_ts/t1", paste0("pool_t1_test_sample", id, ".Rdata")), compress = TRUE)
```

# Fit combined super learner

We fit the Combined Super Learner, Individual Super Learner and the 
Regular Super Learner across a range
of training data lengths:

* t=(30, 60, 90, 120, 150, 180, 210, 240, 270, 300)

Ideally, we would like to see the Combined SL
...
```{r, eval = FALSE, echo = FALSE, warning = FALSE}
##########################################
# Combined time-series with t=1
##########################################
load(here::here("Results/Nonstationary_ts/t1", "full_t1_test_sample20403.Rdata"))

source(here::here("R/CombinedOnlineSL.R"))
source(here::here("R/utils_mimic.R"))

train_all <- nonstat_sub_df
...
comSLsim_res <- lapply(times, function(x){
  if(x %in% c(60,120,180,240,300)){
    combine_SL(t = x, train_all = train_all, outcome=outcome, 
             ...
             id=id, cv="folds_rolling_origin",
             gap=0, h=15, test_size=15, mini_batch=5, first_window=15)
  }else{
    combine_SL(t = x, train_all = train_all, outcome=outcome, 
             stack_pool=stack, stack_individual=stack, stack_screen = stack_screen, sl=sl,
             covars=covars, covars_baseline = covars_baseline,
             id=id, cv="folds_rolling_origin",
             gap=30, h=15, test_size=15, mini_batch=5, first_window=15)
  }
  })

comSLsim_res_cvrw <- lapply(times, function(x){
  if(x %in% c(60,120,180,240,300)){
    combine_SL(t = x, train_all = train_all, outcome=outcome, 
             stack_pool=stack, stack_individual=stack, stack_screen = stack_screen, sl=sl,
             covars=covars, covars_baseline = covars_baseline,
             id=id, cv="folds_rolling_window",
             gap=0, h=15, test_size=15, mini_batch=5, window_size=15)
  }else{
    combine_SL(t = x, train_all = train_all, outcome=outcome, 
             stack_pool=stack, stack_individual=stack, stack_screen = stack_screen, sl=sl,
             covars=covars, covars_baseline = covars_baseline,
             id=id, cv="folds_rolling_window",
             gap=30, h=15, test_size=15, mini_batch=5, window_size=15)
  }
})

save(comSLsim_res, file = here::here("Results/Nonstationary_ts/SL_fits", paste0("CombinedTS_results_t1_cvro_", id, ".Rdata")), compress = TRUE)
save(comSLsim_res_cvrw, file = here::here("Results/Nonstationary_ts/SL_fits", paste0("CombinedTS_results_t1_cvrw_", id, ".Rdata")), compress = TRUE)
```

```{r, eval = FALSE, echo = FALSE, warning = FALSE}
##########################################
# Individual time-series with t=1
##########################################
load(here::here("Results/Nonstationary_ts/t1", "ind_test_t1_sample20403.Rdata"))

source(here::here("R/CombinedOnlineSL.R"))
source(here::here("R/utils_mimic.R"))

train_all <- ind_sub_df
#factor_to_indicators...
id=20403
...
save(comSLsim_res, file = here::here("Results/Nonstationary_ts/SL_fits", paste0("IndividualTS_results_cvro_", id, ".Rdata")), compress = TRUE)
save(comSLsim_res_cvrw, file = here::here("Results/Nonstationary_ts/SL_fits", paste0("IndividualTS_results_cvrw_", id, ".Rdata")), compress = TRUE)
```

# Results for the combined (non-stationary) time-series with t=1

The combined time-series, we mixed it at only at `t=1`. As such, we 
expect the Combined Super Learner to switch from giving more 
weights to the Individual SL for the first half of the time-series, 
than the Combined SL until the end of the simulated time-series. 

We report the following:

* Visual illustration of the combined time-series
* Learner with the minimial loss at each training time with 
  * Rolling Origin Cross-Validation
  * Rolling Window Cross-Validation
* Comparison of the Combined, Individual and Regular Super Learner at 
each training time. 

```{r, eval = TRUE, echo = FALSE, warning = FALSE}
load(here::here("Results/Nonstationary_ts/t1","full_test_t1_sample20403.Rdata"))
load(here::here("Results/Nonstationary_ts/t1","test_t1_sample20403.Rdata"))
...
load(here::here("Results/Nonstationary_ts/SL_fits", "CombinedTS_results_t1_cvro_20403.Rdata"))
load(here::here("Results/Nonstationary_ts/SL_fits", "CombinedTS_results_t1_cvrw_20403.Rdata"))
times <-as.list(seq(30, 300, by = 30))
...
comSLsim_loss_cvrw <- lapply(seq(comSLsim_res_cvrw), function(i){
  names(which.min(comSLsim_res_cvrw[[i]]$loss[[1]]))
})
comSLsim_loss_cvrw <- cbind.data.frame(time=unlist(times), learner=unlist(comSLsim_loss_cvrw))

comSLsim_loss %>%
  kable(format = "latex", booktabs = T, digits = 4, longtable = T,
        caption = "Picked learner at varying training size with Rolling Origin CV") %>%
  kable_styling(latex_options = c("striped", "repeat_header"), font_size = 7,
                position = "center")
comSLsim_loss_cvrw %>%
  kable(format = "latex", booktabs = T, digits = 4, longtable = T,
        caption = "Picked learner at varying training size with Rolling Window CV") %>%
  kable_styling(latex_options = c("striped", "repeat_header"), font_size = 7,
                position = "center")


### Compare Combine SuperLearner at each time-point
comSLsim_loss_all <- lapply(seq(comSLsim_res), function(i){
  comSLsim_res[[i]]$losses_all
})
comSLsim_loss_all <- cbind.data.frame(time=unlist(times), matrix(unlist(comSLsim_loss_all),nrow = length(comSLsim_res),ncol = 3))
names(comSLsim_loss_all)<-c("time","loss_online_SL", "loss_regular_SL", "loss_individual_SL")

comSLsim_loss_all_cvrw <- lapply(seq(comSLsim_res_cvrw), function(i){
  comSLsim_res_cvrw[[i]]$losses_all
})
comSLsim_loss_all_cvrw <- cbind.data.frame(time=unlist(times), matrix(unlist(comSLsim_loss_all_cvrw),nrow = length(comSLsim_res_cvrw),ncol = 3))
names(comSLsim_loss_all_cvrw)<-c("time","loss_online_SL", "loss_regular_SL", "loss_individual_SL")

comSLsim_loss_all %>%
  kable(format = "latex", booktabs = T, digits = 16, longtable = T,
        caption = "Picked SL at varying training size with Rolling Origin CV") %>%
  kable_styling(latex_options = c("striped", "repeat_header"), font_size = 7,
                position = "center")
comSLsim_loss_all_cvrw %>%
  kable(format = "latex", booktabs = T, digits = 16, longtable = T,
        caption = "Picked SL at varying training size with Rolling Window CV") %>%
  kable_styling(latex_options = c("striped", "repeat_header"), font_size = 7,
                position = "center")
```
