...
```{r, echo = FALSE, message = FALSE, warning = FALSE}
summ_hypo$subject_id <- as.numeric(as.character(summ_hypo$subject_id))
hypo_coef$subject_id <- as.numeric(as.character(hypo_coef$subject_id))
...
hypo_coef_W %>%
  kable(format = "latex", booktabs = T, digits = 2, longtable = T,
  ...
  kable_styling(latex_options = c("striped", "repeat_header"), font_size = 4,
                position = "center")
summ_nonhypo$subject_id <- as.numeric(as.character(summ_nonhypo$subject_id))
nonhypo_coef$subject_id <- as.numeric(as.character(nonhypo_coef$subject_id))
...
# Calculate performance metrics of combined super learner
...
```{r, echo = FALSE, eval = FALSE, message = FALSE, warning = FALSE, fig.align = "center"}
