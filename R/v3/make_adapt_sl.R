slstream <- function(
  ...
  print_timers = TRUE, print_onlineSL = TRUE
){
  ...
  if(length(folds) >= 2){
    ...
    # don't make SL from learners that could not be cv-selector according to dSL
    times <- Reduce(c, lapply(trainSL_fold_fits, '[[', 'time'))
    # add times that would be in v_time to appropriately weight
    times <- c(times, (max(times) + 1):(max(times) + batch))
    ...
    weights <- weights[-c((length(weights)-batch+1):length(weights))]
    # identify learners with non-zero weighted non-NA predictions
    if(any(weights == 0)){
      # don't consider rows w 0 weight when figuring out non-NA columns
      weighted_t_pred <- t_pred[which(weights == 0),]
    } else {
      weighted_t_pred <- t_pred
    }
    nonNA_lrnrs <- which(apply(weighted_t_pred, 2, function(x) !any(is.na(x))))
    nonNA_lrnrs <- colnames(weighted_t_pred)[nonNA_lrnrs]
    t_pred <- t_pred[, nonNA_lrnrs, with = F]
