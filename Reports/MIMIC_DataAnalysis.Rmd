...
source(here::here("R", "summarize_outcome.R"))
source(here::here("R", "CombinedOnlineSL_v2.R"))
source(here::here("R", "CombinedOnlineSL_v2_helper.R"))
load(here::here("Data", "comSLargs.Rdata"))
load(here::here("Data", "data_summaryBinY.Rdata"))
...
First, we create the dataset with corresponding summary measures. 
Note that the decision on how much in advance we predict (and over
which time period) must be made before analysis for now --- 
as the dataset used will depend on the decision. 

Below we create a dataset that summarizes the abpmean for each patient
30 minutes in advance, over a 15 minute window. The summary function 
used to test the algorithm is the Y_sol1:

Hypotensive episode for time t is defined as:
...
Once the episodes are identified for the entire period, we summarize over 
the window. 

\vspace{.2in}
```{r, create_data, echo=FALSE, eval=FALSE}
dat <- comSLargs[[1]]
data <- add_summary_outcome(dat, summary_measure = "Y_sol1")
save(data, file = here::here("Data", "data_summaryBinY.Rdata"), 
     compress = TRUE)
```
...
```{r, examine_data_2, results='asis', echo=FALSE, eval=FALSE}
...
```{r, examine_data_3, results='asis', echo=FALSE, eval=FALSE}
...
covars_timevarying <- c("amine", "sedation", "ventilation", "spo2", "time", 
...
comSL_cvrw <- lapply(times, function(x){
  combine_SL(train_all = dat, outcome, t = x, stack_pool = cv_stack, 
             stack_individual = cv_stack, stack_screen = cv_stack_screen, sl = sl,
             covars = covars_timevarying, covars_baseline = covars_baseline,
             cv = "folds_rolling_window", 
             gap_training = 30, h = 15, test_size = 15, mini_batch = 5, window_size = 15,
             outcome_summary=FALSE, adapt_covs=c("gender","rank_icu"))
  })
...
```{r, res_SL_origin, results='asis', echo=FALSE, eval=FALSE}
