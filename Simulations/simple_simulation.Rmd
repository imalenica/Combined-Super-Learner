...
For each subject and each outcome of interest, we display of the model
parameters and multiple measures of accuracy of the model fit: mean error
(`ME`), root mean squared error (`RMSE`), mean absolute error (`MAE`), mean
percentage error (`MPE`), mean absolute percentage error (`MAPE`), mean absolute
scaled error (`MASE`) and the first-order autocorrelation coefficient (`ACF1`).

Auto ARIMA takes into account the AIC and BIC values generated to determine the
best combination of parameters. AIC (Akaike Information Criterion) and BIC
(Bayesian Information Criterion) values are estimators to compare models.

We can incorporate exogenous regressors with ARMA errors where the regression
on the exogenous regressors is done first, and then what remains is given ARIMA
structure.
...
auto_arima <- function(df, outcome) {
  ...
  fit_list <- lapply(samples, function(x){
    ...
    outcome_index <- which(colnames(sub) == outcome)
    # covars <- data.matrix(select(sub, c(amine, ventilation, sedation)),
    #                       rownames.force = TRUE)[,-1]
    # cov_unique <- apply(covars, 2, function(x) length(unique(x)))
    # covs <- which(cov_unique > 1)
    # covs <- covars[,names(covs)]
    fit <- forecast::auto.arima(sub[,outcome_index])
    fit_coef <- fit$coef
    fit_accuracy <- forecast::accuracy(fit)
    return(c(fit_coef, fit_accuracy))
  })
  names(fit_list) <- samples
  fit_coef_all <- do.call("rbind.fill",
...
train_all <- sample_n_t(new_dat, n = 500, t = 5)
