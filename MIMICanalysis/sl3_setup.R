...
make_historical_stack <- function(){
  ...
  W <- c("sex", "age", "care_unit", "sapsi_first", "sofa_first",
...
make_individual_cv_stack <- function() {
  ...
  lrnrs_no_screen <- c(lrn_glm, arima_aicc, arima_bic)
  names(lrnrs_no_screen) <- c("glm", "arima_aicc", "arima_bic")
  ...
  ##### put it all together
  lrnrs <- c(glmnets, xgb, rf, nlts_setar, nlts_nnet5, nlts_nnet15, lrn_mean,
             pipe_screen)
  names(lrnrs) <- c(
