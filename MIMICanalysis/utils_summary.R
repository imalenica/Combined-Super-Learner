  ...
  for (y in 1:length(outcome_types)){
    ...
    for (s in 1:length(smooth_types)){
      ...
      for(i in 1:length(outcomes)){
        ...
        # MSE summary
        time_idx <- grepl("time", colnames(result$performance_table))
        perf_tbl <- result$performance_table[, -time_idx, with=F]
        mean_performance[[i]] <- data.table(t(round(colMeans(perf_tbl, na.rm=T),4)))
        median_performance[[i]] <- data.table(t(round(apply(perf_tbl, 2, median, na.rm=T),4)))
    ...
    if(outcome_type == "binomial"){
      ...
      nums <- data.table(apply(nums, 2, function(x) round(as.numeric(x), 4)))
